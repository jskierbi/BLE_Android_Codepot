...
import com.zinno.sensortaglibrary.ble.BleGattExecutor;
...
public abstract class TiSensor<T> {
    ...
    public static String CHARACTERISTIC_CONFIG = "00002902-0000-1000-8000-00805f9b34fb";
    ...
    public BleGattExecutor.ServiceAction read(BluetoothGatt gatt, final String uuid) {
        return new BleGattExecutor.ServiceAction(gatt, BleGattExecutor.ServiceAction.ActionType.READ) {
            @Override
    ...
    public BleGattExecutor.ServiceAction write(BluetoothGatt gatt, final String uuid, final byte[] value) {
        return new BleGattExecutor.ServiceAction(gatt, BleGattExecutor.ServiceAction.ActionType.WRITE) {
            @Override
            public boolean execute(BluetoothGatt bluetoothGatt) {
                final BluetoothGattCharacteristic characteristic = getCharacteristic(bluetoothGatt, uuid);
    ...
    public BleGattExecutor.ServiceAction notify(BluetoothGatt gatt, final boolean start) {
        return new BleGattExecutor.ServiceAction(gatt, BleGattExecutor.ServiceAction.ActionType.NOTIFY) {
            @Override
            public boolean execute(BluetoothGatt bluetoothGatt) {
